...
dump, dump2, news = [], [], []
sys.stdout.write('\x1b]2; Facebook Craking | Rul Senpai\x07')
...
class REQ:
   ...
   def UserAgent(self):
       ...
       # Generate UserAgent
       self.ua1 = f"Mozilla/5.0 (Linux; Android 10; Redmi Note 9 Pro Build/QKQ1.191215.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/91.0.4472.77 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/325.0.0.36.170;]"
       self.ua2 = f"Mozilla/5.0 (Linux; Android 11; Samsung Galaxy Build/RP1A.200720.012) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.110 Mobile Safari/537.36"
       self.ua3 = f"Mozilla/5.0 (Linux; Android 11; Redmi Build/RKQ1.200826.002) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/96.0.4664.110 Mobile Safari/537.36"
   ...
   def Logooo(self):
       Console().print(Markdown(f"## Facebook Cracking. Rul Senpai"),style='bold yellow')
   ...
   def REQ_Cookie(self):
       ...
       Console().print(Markdown(f"## Masukan cookie  anda, pastikan anda menggunakan akun tumbal"),style='white')
                ...
                for z in self.all_friends:
                    ...
                    if len(self.name) == 0 or len(self.name) > 100:
                        ...
                        if str(self.id_friends) in str(dump):
                            ...
                            Console().print(f'>Rul Senpai -: {K2}{len(dump)}{P2}',end='\r')
        ...
        try:
            for xyc in range(int(total)):
                ...
                if data in dump:pass
                    ...
                    Console().print(f'>Rul Senpai-: {H2}{len(dump)}.{len(data)}{P2}',end='\r')
        ...
        try:
            ...
            for data in Ambil:
                try:
                    ...
                    Console().print(Markdown(f"## {str(e).title()}"),style='red') ; sleep(3.1) ; sys.exit() 
                Console().print(f'>Rul Senpai -: {H2}{len(dump)}{P2}',end='\r')
        except FileNotFoundError as e:
            Console().print(Markdown(f"## {str(e).title()}"),style='red') ; sleep(3.1) ; sys.exit()
...
class Furthermore:
   ...
   def asyncAPI(self, user, password):
       prog.update(des,description=f">Rul Senpai | {self.loop}/{len(dump)} LIVE-:{H2}{len(self.live)} {P2}CHECK-:{K2}{len(self.check)}")
       prog.advance(des) 
